...
class QRCode {
    ...
    var int $typeNumber {
        ...
        set(int $value) => $this->typeNumber = $value;
    }
    ...
    var int $errorCorrectLevel {
        get {
            return $this->errorCorrectLevel;
        }
        set(int $value) => $this->errorCorrectLevel = $value;
    }

    var array $qrDataList;
    ...
    function addData($data, $mode = 0): void
    {
    ...
    function addDataImpl($qrData): void
    {
        $this->qrDataList[] = $qrData;
    }

    function getDataCount(): int
    {
    ...
    function hex2rgb($hex = 0x0): array
    {
        return array(
            'r' => floor($hex / 65536),
            'g' => floor($hex / 256) % 256,
    ...
    function getBestMaskPattern(): int
    {
    ...
    function createNullArray($length): array
    {
        $nullArray = array();
        for ($i = 0; $i < $length; $i++) {
            $nullArray[] = null;
    ...
    function makeImpl($test, $maskPattern): void
    {
    ...
    function mapData(&$data, $maskPattern): void
    {
    ...
    function setupPositionProbePattern($row, $col): void
    {
    ...
    function setupTypeNumber($test): void
    {
    ...
    function setupTypeInfo($test, $maskPattern): void
    {
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray): array
    {
     ...
     * @return array
     */
    function createBytes(QRBitBuffer &$buffer, array &$rsBlocks): array
    {
    ...
    static function getMinimumQRCode($data, $errorCorrectLevel): QRCode
    {
    ...
    function printHTML($size = "2px"): void
    {
    ...
    public function printSVG($size = 2): void
    {
        $width = $this->moduleCount * $size;
        $height = $width;
...
class QRUtil {
    ...
    static array $QR_MAX_LENGTH = array(
    ...
    static array $QR_PATTERN_POSITION_TABLE = array(
        array(),
        array(6, 18),
        array(6, 22),
        array(6, 26),
    ...
    static function getErrorCorrectPolynomial($errorCorrectLength): QRPolynomial
    {
     ...
     * @return float|int
     */
    static function getLostPoint(QRCode $qrCode): float|int
    {
    ...
    static function isNumber($s): bool
    {
    ...
    static function isAlphaNum($s): bool
    {
        for ($i = 0; $i < strlen($s); $i++) {
            $c = ord($s[$i]);
            if (!(QRUtil::toCharCode('0') <= $c && $c <= QRUtil::toCharCode('9') )
    ...
    static function isKanji($s): bool
    {
    ...
    static function toCharCode($s): int
    {
        return ord($s[0]);
    }

    static function getBCHTypeInfo($data): int
    {
        $d = $data << 10;
    ...
    static function getBCHTypeNumber($data): int
    {
        $d = $data << 12;
    ...
    static function getBCHDigit($data): int
    {
...
class QRRSBlock {
    ...
    static array $QR_RS_BLOCK_TABLE = array(
    ...
    static function getRSBlocks($typeNumber, $errorCorrectLevel): array
    {
...
class QRNumber extends QRData {
    ...
    function write(QRBitBuffer &$buffer): void
    {
...
abstract class QRData {
     ...
     * @param \QRBitBuffer $buffer
     */
    abstract function write(QRBitBuffer &$buffer);
...
class QRMath {
    ...
    static function createNumArray($length): array
    {
        $num_array = array();
        for ($i = 0; $i < $length; $i++) {
            $num_array[] = 0;
...
class QRPolynomial {
     ...
     * @return \QRPolynomial
     */
    function multiply(QRPolynomial $e): QRPolynomial
    {
     ...
     */
    function mod(QRPolynomial $e): QRPolynomial|static
    {
...
class QRBitBuffer {
    ...
    function get($index): bool
    {
        $bufIndex = (int)floor($index / 8);
        return ( ($this->buffer[$bufIndex] >> (7 - $index % 8) ) & 1) == 1;
    }

    function put($num, $length): void
    {
